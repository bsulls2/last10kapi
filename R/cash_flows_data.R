#' Create a data frame of a company's cash flow statement.
#'
...
#'
#' @return A data frame of the chosen company's cash flow statement line items.
#'
#' @export

cash_flows_data <- function(ticker, form_type, filing_order, subscription_key) {
  url <- paste("https://services.last10k.com/v1/company", ticker,
               "cashflows?formType=", sep = "/")
  ...
  cf <- map_df(data_url$data$attributes$result, extract) %>%
